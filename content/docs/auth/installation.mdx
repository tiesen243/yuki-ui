...
By default, Auth uses [`Scrypt`](https://github.com/pilcrowonpaper/oslo/blob/main/src/password/scrypt.ts) for hashing passwords. You can change the hashing algorithm by passing a different hashing function to the `hash` and `verify` methods in the `server/auth/core/password.ts` file.
