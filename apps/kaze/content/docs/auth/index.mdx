...
<Tabs items={['Memory', 'Drizzle', 'Prisma', 'Mongoose']}>
  <Tab value='Memory'>
    <InstallComponent comp='auth' />
  </Tab>
  <Tab value='Drizzle'>
    <InstallComponent comp='auth-adapter-drizzle' />
  </Tab>
  <Tab value='Prisma'>
    ...
    To handle authentication requests, create an API route in your server framework of choice and export the `handler` function from the authentication package.
      ...
      <Tabs items={['Next.js', 'React Router', 'Tanstack Router', 'Express', 'Elysia', 'Nitro']}>
          ...
          export const Route = createFileRoute('/api/auth/$')({
            server: {
              handlers: {
                ...
                OPTIONS: async ({ request }) => handlers(request),
              },
            },
          })
          ```
        ...
        <Tab value='Nitro'>
          ...
          export default defineEventHandler(event => handlers(event.node.req))
          ```
        </Tab>
