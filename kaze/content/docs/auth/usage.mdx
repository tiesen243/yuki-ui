...
1. Implement in server-side API route to throw an error when the access token is expired.
...
2. Use the `retry` option in `useQuery` to attempt refreshing the token when an error occurs.
...
If you use `tRPC` for your API routes, you can use the `retryLink` to handle token refresh logic globally for all tRPC requests.
