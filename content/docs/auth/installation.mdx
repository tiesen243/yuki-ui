...
<Tabs items={['Drizzle', 'Prisma', 'MongoDb','Memory']}>
...
import * as schema from '@/serer/db/schema'
...
export const db = globalForMemory.db ?? {
  users: [],
  accounts: [],
  sessions: [],
}
if (process.env.NODE_ENV !== 'production') globalForMemory.db = db
```
...
By default, Auth uses [`Scrypt`](https://github.com/lucia-auth/lucia/blob/v3/packages/lucia/src/scrypt/index.ts) for hashing passwords. You can change the hashing algorithm by passing a different hashing function to the `hash` and `verify` methods in the `server/auth/core/password` folder.
...
<InstallComponent name="oauth-{provider}" />
...
<InstallComponent name="oauth-google" />
...
If you're implementing Auth in a framework other than Next.js, remember to remove any imports of `cookies` from `next/headers` and replace them with the appropriate cookie management solution for your framework.
...
async function getCookie(
  key: string,
  request: Request, // make sure the `req` parameter is required
): Promise<string | undefined> {
  const cookies = request.headers
    .get('cookie')
    ?.split(';')
